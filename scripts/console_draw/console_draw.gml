...
if c[? "active"] {
    ...
    /* Get line height */
    var l,p;
    l = string_height(string_hash_to_newline("W"));
    p = 8; // Padding
    ...
    for (ch=0;ch<round(h/l);ch++) {
        ...
        var cmd = list[| ch];
        if !is_undefined(cmd) {
            draw_text(p,h-(p*2)-(ch*l),string_hash_to_newline(cmd));
        }
    ...
    && ds_list_size(c[? "suggestions"]) > 0 {
        ...
        for(s=0;s<ds_list_size(sugs);s++) {
            if !is_undefined(sugs[| s]) {
                draw_text(p,((h+(p*5))+l)+(l*s),string_hash_to_newline(sugs[| s]));
            }
        }
    }
    ...
    draw_set_color(t1);
    draw_text(p,h+(p*3),string_hash_to_newline("> "));
    
    /* Draw input */
    draw_set_alpha(1);
    draw_set_color(t2);
    draw_text(p+string_width(string_hash_to_newline("> ")),h+(p*3),string_hash_to_newline(c[? "string"]));
    ...
    /* Draw cursor */
    draw_text(p+string_width(string_hash_to_newline("> "))+string_width(string_hash_to_newline(string_copy(c[? "string"],1,c[? "string_pos"]-1))),h+(p*3),string_hash_to_newline("|"));
